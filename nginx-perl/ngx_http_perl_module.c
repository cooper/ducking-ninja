...
#define LOG_(x) ngx_log_error(NGX_LOG_NOTICE, log, 0, x)
...
ngx_http_perl_run_requires(pTHX_ ngx_array_t *requires, ngx_log_t *log)
{
    ...
    LOG_("ngx_http_perl_run_requires() called");

    if (requires == NGX_CONF_UNSET_PTR) {
        LOG_("ngx_http_perl_run_requires() requires = NGX_CONF_UNSET_PTR");
        return NGX_OK;
    }
    ...
    LOG_("ngx_http_perl_run_requires() returning OK");
